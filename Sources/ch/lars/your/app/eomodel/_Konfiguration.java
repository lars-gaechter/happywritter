...
public abstract class _Konfiguration extends ERXGenericRecord {
	...
	public static Konfiguration createKonfiguration(EOEditingContext editingContext) {
		Konfiguration eo = (Konfiguration) EOUtilities.createAndInsertInstance(editingContext,
				_Konfiguration.ENTITY_NAME);
		return eo;
	}
	...
	public static NSArray<Konfiguration> fetchKonfigurations(EOEditingContext editingContext, EOQualifier qualifier,
			NSArray<EOSortOrdering> sortOrderings) {
		ERXFetchSpecification<Konfiguration> fetchSpec = new ERXFetchSpecification<Konfiguration>(
				_Konfiguration.ENTITY_NAME, qualifier, sortOrderings);
	...
	public static Konfiguration fetchKonfiguration(EOEditingContext editingContext, EOQualifier qualifier) {
		...
		if (count == 0) {
			...
			throw new IllegalStateException(
					"There was more than one Konfiguration that matched the qualifier '" + qualifier + "'.");
		}
		return eoObject;
	}

	public static Konfiguration fetchRequiredKonfiguration(EOEditingContext editingContext, String keyName,
			Object value) {
	...
	public static Konfiguration fetchRequiredKonfiguration(EOEditingContext editingContext, EOQualifier qualifier) {
		...
		if (eoObject == null) {
			throw new NoSuchElementException(
					"There was no Konfiguration that matched the qualifier '" + qualifier + "'.");
		}
		return eoObject;
	}

	public static Konfiguration localInstanceIn(EOEditingContext editingContext, Konfiguration eo) {
		Konfiguration localInstance = (eo == null) ? null
