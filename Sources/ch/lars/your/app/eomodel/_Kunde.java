...
public abstract class _Kunde extends ERXGenericRecord {
	...
	// Relationship Keys
	public static final ERXKey<ch.lars.your.app.eomodel.Bestellung> BESTELLUNGS = new ERXKey<ch.lars.your.app.eomodel.Bestellung>(
			"bestellungs", Type.ToManyRelationship);
	...
	public NSArray<ch.lars.your.app.eomodel.Bestellung> bestellungs(EOQualifier qualifier,
		...
		if (fetch) {
			...
			results = ch.lars.your.app.eomodel.Bestellung.fetchBestellungs(editingContext(), fullQualifier,
					sortOrderings);
		} else {
			results = bestellungs();
			if (qualifier != null) {
				results = (NSArray<ch.lars.your.app.eomodel.Bestellung>) EOQualifier.filteredArrayWithQualifier(results,
						qualifier);
			}
			if (sortOrderings != null) {
				results = (NSArray<ch.lars.your.app.eomodel.Bestellung>) EOSortOrdering
	...
	public ch.lars.your.app.eomodel.Bestellung createBestellungsRelationship() {
		EOEnterpriseObject eo = EOUtilities.createAndInsertInstance(editingContext(),
				ch.lars.your.app.eomodel.Bestellung.ENTITY_NAME);
